...
struct PLAYER_NAME : public Player {
  ...
  void night_builder (const int& id) {
    ...
    if ((cell(p).b_owner == me()) or (citizen(id).life < builder_ini_life() and cell(p).bonus == Food) or (cell(p).bonus == Money)) return;
    ...
    while(to_visit_cells.size() > 0) {
      ...
      if ((cell(possible_cell).b_owner == me() and cell(possible_cell).id == -1) or (citizen(id).life < builder_ini_life() and cell(possible_cell).bonus == Food) or (cell(possible_cell).bonus == Money)) {
        move(id, possible_dir);
        return;
      }
